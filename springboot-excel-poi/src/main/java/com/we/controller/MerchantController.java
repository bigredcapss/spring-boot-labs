...
public class MerchantController {
    ...
    @GetMapping(value = "/merchant/export")
    @ResponseBody
    public String exportExcel(HttpServletResponse response,String search){

        try {
            search= !StringUtils.isEmpty(search)? URLDecoder.decode(search,"UTF-8"):null;

            String[] headers = new String[]{"商户号","商户名称","商户地址","账号","户名","商户状态"};
            List<Merchant> merchants = merchantService.getMerchantList();

            List<Map<Integer, Object>> dataList = ExcelBeanUtil.manageObjectList(merchants);
            HSSFWorkbook workbook = new HSSFWorkbook();
            ExcelUtil.fillExcelSheetData(dataList,workbook,headers,"商户信息表");
            WebUtil.downloadExcel(response, workbook, "商户信息表.xls");
            return "商品信息表.xls";
        }catch (Exception e){
